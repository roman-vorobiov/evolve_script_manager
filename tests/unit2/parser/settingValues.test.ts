        ...
        describe("Setting values", () => {
            describe("Constant literals", () => {
                it.each([
                    ...
                    const expectedNode = maps(`foo = ${source}`, <Parser.SettingAssignment> {
                        ...
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
                        value: maps(source, { type, value: target })
                    });
            ...
            describe("Identifiers", () => {
                it("should parse identifiers", () => {
                    ...
                    const expectedNode = maps(`foo = bar`, <Parser.SettingAssignment> {
                        type: "SettingAssignment",
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
            ...
            describe("Subscript", () => {
                ...
                it("should parse a list of identifiers", () => {
                    const { nodes, errors, maps } = parse(`foo = bar[aaa, bbb, ccc]`);

                    expect(errors).toStrictEqual([]);
                    expect(nodes.length).toBe(1);

                    const expectedNode = maps(`foo = bar[aaa, bbb, ccc]`, <Parser.SettingAssignment> {
                        type: "SettingAssignment",
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
                        value: maps("bar[aaa, bbb, ccc]", {
                            type: "Subscript",
                            base: maps("bar", { type: "Identifier", value: "bar" }),
                ...
                it("should parse a list of identifiers (disjunction)", () => {
                    const { nodes, errors, maps } = parse(`foo = bar[aaa, bbb or ccc]`);

                    expect(errors).toStrictEqual([]);
                    expect(nodes.length).toBe(1);

                    const expectedNode = maps(`foo = bar[aaa, bbb or ccc]`, <Parser.SettingAssignment> {
                        type: "SettingAssignment",
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
                        value: maps("bar[aaa, bbb or ccc]", {
                            type: "Subscript",
                            base: maps("bar", { type: "Identifier", value: "bar" }),
                ...
                it("should parse a list of identifiers (conjunction)", () => {
                    const { nodes, errors, maps } = parse(`foo = bar[aaa, bbb and ccc]`);

                    expect(errors).toStrictEqual([]);
                    expect(nodes.length).toBe(1);

                    const expectedNode = maps(`foo = bar[aaa, bbb and ccc]`, <Parser.SettingAssignment> {
                        type: "SettingAssignment",
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
                        value: maps("bar[aaa, bbb and ccc]", {
                            type: "Subscript",
                            base: maps("bar", { type: "Identifier", value: "bar" }),
                ...
                it("should parse nested subscripts", () => {
                    const { nodes, errors, maps } = parse(`foo = aaa[bbb[ccc.ddd]]`);

                    expect(errors).toStrictEqual([]);
                    expect(nodes.length).toBe(1);

                    const expectedNode = maps(`foo = aaa[bbb[ccc.ddd]]`, <Parser.SettingAssignment> {
                        type: "SettingAssignment",
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
                        value: maps("aaa[bbb[ccc.ddd]]", {
                            type: "Subscript",
                ...
                it("should parse a list of nested subscripts", () => {
                    const { nodes, errors, maps } = parse(`foo = bar[aaa.bbb, ccc[ddd, eee]]`);

                    expect(errors).toStrictEqual([]);
                    expect(nodes.length).toBe(1);

                    const expectedNode = maps(`foo = bar[aaa.bbb, ccc[ddd, eee]]`, <Parser.SettingAssignment> {
                        type: "SettingAssignment",
                        setting: maps("foo", { type: "Identifier", value: "foo" }),
                        value: maps("bar[aaa.bbb, ccc[ddd, eee]]", {
                            type: "Subscript",
                            base: maps("bar", { type: "Identifier", value: "bar" }),
